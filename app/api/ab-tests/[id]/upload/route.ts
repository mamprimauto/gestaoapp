...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  // Permission check: verify user can access this A/B test
  {
  ...
  // Generate safe path for A/B test comments
  const ext = (fileName.split(".").pop() || "bin").toLowerCase()
  const safeBase = fileName.replace(/[^\w.\-\s()[\]]+/g, "_").slice(0, 140)
  const path = `ab-tests/${testId}/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`
