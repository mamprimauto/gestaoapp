...
interface OKRTaskCommentsProps {
  taskId: string
  onCommentChange?: () => void
}

export default function OKRTaskComments({ taskId, onCommentChange }: OKRTaskCommentsProps) {
  ...
  const loadComments = useCallback(async () => {
    ...
    try {
      ...
      if (!session?.access_token) {
        ...
        const res = await fetch(`/api/okr/tasks/${taskId}/comments`, {
  ...
  const uploadFile = useCallback(async (file: File): Promise<string> => {
    try {
      ...
      const signRes = await fetch(`/api/okr/tasks/${taskId}/upload`, {
        method: "POST",
        headers: {
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <label
                htmlFor="okr-comment-file-upload"
                className={cn(
