-- ========================================
-- SOLUÇÃO AUTOMÁTICA CORRIGIDA: Criar organização para TODOS os usuários
-- Este script garante que TODOS os usuários tenham uma organização
-- VERSÃO CORRIGIDA: Usa campo 'name' ao invés de 'full_name'
-- ========================================
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    INSERT INTO organizations (name, slug, description, created_by)
    VALUES (
      COALESCE(NEW.name, 'User') || '''s Workspace',
...
DROP POLICY IF EXISTS "users_create_orgs" ON organizations;
DROP POLICY IF EXISTS "users_update_own_orgs" ON organizations;
DROP POLICY IF EXISTS "users_see_org_members" ON organization_members;
...
-- Verificar se track_records tem RLS desabilitado (temporário)
SELECT 
  schemaname,
  tablename,
  rowsecurity
FROM pg_tables
WHERE tablename IN ('track_records', 'track_record_variations', 'track_record_kpis', 'organizations', 'organization_members');

-- ========================================
-- IMPORTANTE: Se ainda houver problemas com track_records
-- Execute também isto para garantir que funcione:
-- ========================================
...
-- 4. Se necessário, o RLS de track_records está desabilitado no final
-- ========================================