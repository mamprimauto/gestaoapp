-- ========================================
-- SIMPLIFICAR SISTEMA DE TRACK RECORDS (VERSÃO FINAL CORRIGIDA)
-- Remove organizações, limpa dados duplicados, corrige função
-- ========================================
...
ALTER TABLE IF EXISTS track_records DROP CONSTRAINT IF EXISTS track_records_track_record_id_unique;

-- PASSO 5: LIMPAR DUPLICADOS antes de remover organization_id
DO $$
DECLARE
...
BEGIN
    ...
    IF duplicate_count > 0 THEN
        -- Deletar registros duplicados (mantém apenas o mais recente por track_record_id)
...
-- PASSO 6: Agora remover a coluna organization_id com CASCADE
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='track_records' AND column_name='organization_id') THEN
        ...
        RAISE NOTICE 'Coluna organization_id removida com CASCADE';
    END IF;
END $$;

-- PASSO 7: Criar constraint unique no track_record_id (agora sem duplicados)
ALTER TABLE track_records ADD CONSTRAINT track_records_track_record_id_unique UNIQUE(track_record_id);

-- PASSO 8: Atualizar função para gerar IDs sem organization_id (CORRIGIDA - sem ambiguidade)
CREATE OR REPLACE FUNCTION get_next_track_record_id(p_test_type TEXT, p_test_year INTEGER)
RETURNS TEXT AS $$
DECLARE
    next_seq INTEGER;
    formatted_id TEXT;
BEGIN
    -- Busca o próximo número sequencial para o tipo e ano (usando alias tr)
    -- Parâmetros prefixados com p_ para evitar ambiguidade
    SELECT COALESCE(MAX(
        CASE 
            WHEN tr.track_record_id ~ ('^' || p_test_type || '-' || p_test_year || '-[0-9]+$') 
            THEN CAST(SUBSTRING(tr.track_record_id FROM '[0-9]+$') AS INTEGER)
            ELSE 0
        END
    ), 0) + 1
    INTO next_seq
    ...
    formatted_id := p_test_type || '-' || p_test_year || '-' || LPAD(next_seq::TEXT, 3, '0');
    
    RETURN formatted_id;
END;
$$ LANGUAGE plpgsql;

-- PASSO 9: Recriar view track_record_summary SEM organization_id
...
-- PASSO 10: Criar políticas SUPER SIMPLES para TODAS as tabelas (apenas usuário autenticado)
-- Track Records
...
-- PASSO 11: Reabilitar RLS com políticas simples
...
-- PASSO 12: Garantir permissões básicas
...
-- PASSO 13: Atualizar comentários
...
-- Mostrar quantos registros restaram após limpeza
SELECT COUNT(*) as total_track_records FROM track_records;
...
-- Verificar se ainda há duplicados (deve retornar 0)
SELECT 
    track_record_id, 
    COUNT(*) as count 
FROM track_records 
...
-- Testar função (CORRIGIDA - usando novos nomes de parâmetros)
SELECT get_next_track_record_id('VSL', 2025) as next_test_id;

-- Mostrar alguns registros para verificar
SELECT track_record_id, hypothesis, created_at FROM track_records ORDER BY created_at DESC LIMIT 5;