...
export default function NovoBriefingCriativoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Briefing de criativos criado com sucesso!")
      router.push('/swipe-file/briefings')
    } catch (error: any) {
      if (error.code === '42P01') {
        toast.error("Tabela briefings_criativos não existe. Execute o script SQL no Supabase.")
      } else {
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-xl font-bold text-white">Novo Briefing de Criativos</h1>
              </div>
            </div>
            <Button
              type="submit"
        ...
        <form id="briefing-form" onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="pt-6 border-t border-gray-800">
            <Button
              type="submit"
              disabled={loading}
              className="w-full bg-blue-600 hover:bg-blue-700 text-white h-12 md:hidden"
            >
              <Save className="h-4 w-4 mr-2" />
              {loading ? "Salvando..." : "Salvar Briefing"}
            </Button>
