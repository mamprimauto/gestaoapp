...
function KanbanCard({ task, workspace, members, onTaskClick, onDelete, onDuplicate }: KanbanCardProps) {
    ...
    >
      <Card
        className={cn(
          ...
          isOverdue && "ring-2 ring-[#4A4A4C] bg-[#2A2A2C]",
      ...
      >
        <CardContent className="p-4">
          ...
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1 min-w-0">
              <h4 className="font-semibold text-white/90 text-sm leading-tight line-clamp-2 mb-2 group-hover:text-white transition-colors">
              {task.title}
            </h4>
            </div>
        ...
        {/* Descrição (se houver) */}
        {task.description && !taskData.checklist && (
        ...
        <div className="flex items-center justify-between pt-3 mt-2 border-t border-[#2A2A2C]">
          ...
          {assignee ? (
            <Avatar className="h-7 w-7 border-2 border-[#2A2A2C] shadow-sm ring-1 ring-[#3A3A3C]">
            ...
            <div className="h-7 w-7 rounded-full border-2 border-dashed border-[#3A3A3C] flex items-center justify-center hover:border-[#4A4A4C] transition-colors">
