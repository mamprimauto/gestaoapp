-- ============================================
-- MIGRATION: Creative Products (SAFE VERSION)
-- Verifica se estruturas existem antes de criar
-- ============================================
...
INSERT INTO creative_products (name, user_id, description, color, bg_gradient, icon, nomenclature)
SELECT
  name,
  auth.uid(),
  description,
  ...
  nomenclature
FROM (VALUES
  ('Memo Master', 'App de memorização e estudos inteligentes', '#007AFF', 'from-blue-500 to-blue-600', '🧠', 
  ...
  ('Youtasky', 'Gerenciador de tarefas para YouTubers', '#FF453A', 'from-red-500 to-red-600', '📹',
  ...
  ('Alma Gêmea', 'Plataforma de relacionamentos autênticos', '#FF2D92', 'from-pink-500 to-pink-600', '💖',
...
) AS products(name, description, color, bg_gradient, icon, nomenclature)
WHERE NOT EXISTS (
...
SELECT 
  ...
  COUNT(CASE WHEN user_id = auth.uid() THEN 1 END) as seus_produtos
